# M6 人工验收清单（20260917T153031Z_ccb5555）
...
## MANUAL-M5-01 · 四种正式导出
- 时间：2026-09-17T23:21:19.6485751+08:00
- 浏览器 A：Codex in-app browser / 教师与学生页
- 浏览器 B：不适用
- 操作：教师尝试报告 JSON、CSV、Excel 兼容 XML、打印页；学生尝试本人学习记录 JSON（全部课程与当前课程）。
- 预期：四种正式导出和学生 learning-record 均由服务端受控生成。
- 实际：教师 JSON、CSV、Excel 兼容 XML显示生成成功；打印页被浏览器弹窗策略阻止；学生两种范围均返回“当前身份无权导出该范围”。
- 结果：fail
- 证据备注：打印和学生 learning-record 未通过，按真实页面结果记 fail。
...
- 浏览器 A：Codex in-app browser / 127.0.0.1 教师 Origin
- 浏览器 B：Codex in-app browser / localhost 学生 Origin
- 操作：保持 tests/.tmp/m6-manual.sqlite，停止并重启服务；使用 remember Session 恢复页面，检查 QA、analysis、task history、completion。
- 预期：Session 恢复且 QA/reply、analysis、task history、completion 全部仍可从页面读取。
- 实际：Session/CSRF 恢复；QA/reply、v1 task history、1/10 completion 和反馈仍存在；学情研判页却显示当前班级尚未运行导入研判。
- 结果：fail
- 证据备注：任务页仍保留 analysis 来源标识，但 analysis 页面未恢复当前研判，综合回归按 fail 记录。
