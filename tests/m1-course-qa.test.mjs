...
async function withServer(run, options = {}) {
  ...
  try { await run(base); }
  finally { activeFetch=previousFetch; await new Promise(resolve => server.close(resolve)); }
}
...
test("M1-T06 错误教师 context 被服务端拒绝", async () => withServer(async base => { await ask(base,7,"栈和队列的区别是什么？"); const response=await fetch(`${base}/api/qa/teacher-inbox?context=teacher%3A1%3A1`); assert.equal(response.status,403); assert.equal((await response.json()).code,"AUTH_CONTEXT_FORBIDDEN"); }));
...
test("M1-T17 未选修 offering 拒绝", async () => withServer(async base => { const db=new DatabaseSync(dbPath,{readOnly:true}); let id; try{id=db.prepare("SELECT id FROM course_offerings WHERE id NOT IN (SELECT offering_id FROM enrollments WHERE student_id=1) LIMIT 1").get().id}finally{db.close()} const r=await ask(base,id,"什么是课程？"); assert.equal(r.status,403); assert.equal((await r.json()).code,"AUTH_OFFERING_FORBIDDEN"); }));
