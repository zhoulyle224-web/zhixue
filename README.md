...
2. 教师学情研判：直接读取本人所授课程、班级和学生数据，进入只读学生观察视角，生成并确认个人学习计划。

```text
班级数据直读 → 学生画像 → 教师观察 → 个人计划草案
→ 教师确认发布 → 学生答疑与学习 → 效果评估 → 下一轮调整
```
...
当前第二轮自动回归：Node 产品测试 185/185、Skill 31/31，fail 0。历史 M6 人工验收状态仍以 `evidence/m6/` 为准，自动化结果不替代尚未执行的人工项。
...
- 教师观察视角保持教师 Session，只读展示当前学生向当前任课教师开放的数据，不冒充学生身份。
- 学生直接登录端完整保留，学生可独立提问、完成任务并管理可选共享项。
- 个人计划由画像 Skill 生成草案，教师编辑确认后发布，学生端只读取已发布版本。
...
| `/api/teacher/students/:no/observation` | GET | 教师只读观察学生公开数据 |
| `/api/student/sharing-preferences` | GET/PUT | 学生管理可选共享明细 |
| `/api/qa/sessions/*` | POST | 新建会话、连续问答与显式转教师 |
| `/api/plans/*` | GET/POST/PUT | 个人计划草案、编辑、发布与学生读取 |
...
- 第二轮自动化：185/185，fail 0，skip 0，todo 0。
