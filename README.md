...
Windows 发布包解压后可直接双击 `deploy-and-open.bat` 或 `一键部署并打开.bat`。脚本优先使用包内 `runtime/node.exe`，仅在源码目录缺少内置运行环境时才回退到系统 Node.js；它只在 8080～8090 中选择空闲端口或复用当前目录、当前源码指纹完全一致的实例。旧版本仍在运行时会自动换用空闲端口，不会误开旧页面，也不会终止无关进程。

删除某个已启动的版本前，请先在该版本目录双击 `stop-service.bat` 或 `停止智学双擎服务.bat`。停止脚本会核对 PID、端口和实例指纹，只终止当前目录启动的服务，然后释放 Windows 文件锁；不会删除业务数据或其他版本进程。
...
ZHIXUE_MODEL_ALLOWED_HOSTS=你的服务商域名
```

自定义地址必须是 HTTPS 且域名进入服务端允许列表。未配置或调用失败时会明确显示“AI 未生成”，不会把检索文本伪装成大模型回答；密钥不会进入浏览器持久化、Git、普通日志或发布 ZIP。本轮没有更新 CloudBase 版本或控制台配置。
