...
let teacherStudents=[],selectedObservation=null,selectedStudentNo='',personalPlanDraft=null;
let teacherQaState={rows:[],loading:true,error:''};
...
function renderTeacherReports(){
  ...
  const classBox=document.querySelector('#reportClassSummary'),qaBox=document.querySelector('#reportQaSummary'),reviewBox=document.querySelector('#reportReviewSummary');
  if(classBox){
    const data=teacherDashboardData,weakest=[...(knowledge||[])].sort((a,b)=>(Number(a.value)||0)-(Number(b.value)||0))[0];
    classBox.innerHTML=data?`<div class="report-facts"><span><b>${data.studentCount??'--'}</b> 名学生</span><span><b>${valueText(data.averageMastery,'%')}</b> 平均掌握</span><span><b>${valueText(data.passRate,'%')}</b> 及格率</span><span><b>${data.riskCount??0}</b> 名需关注</span></div><p><b>当前薄弱点：</b>${esc(weakest?.name||'暂无')} ${weakest?`${weakest.value}%`:''}</p><button class="link-btn" onclick="openView('analysis')">查看完整学情分析 →</button>`:'<p class="data-empty">当前班级数据尚未加载，请刷新后重试。</p>';
  }
  if(qaBox){
    const rows=teacherQaState.rows||[],pending=rows.filter(x=>x.status==='pending_teacher').length,replied=rows.filter(x=>x.status==='teacher_replied').length,answered=rows.filter(x=>x.status==='answered').length;
    qaBox.innerHTML=teacherQaState.error?`<p class="data-empty">${esc(teacherQaState.error)}</p>`:teacherQaState.loading&&!rows.length?'<p class="data-empty">正在读取答疑记录…</p>':`<div class="report-facts"><span><b>${rows.length}</b> 条记录</span><span><b>${pending}</b> 待回复</span><span><b>${replied}</b> 已回复</span><span><b>${answered}</b> Skill 已答</span></div>${rows.slice(0,3).map(x=>`<p><b>${esc(x.studentAlias||'学生')}：</b>${esc(x.question)}</p>`).join('')||'<p>当前课程暂无答疑记录。</p>'}<button class="link-btn" onclick="openView('inbox')">查看全部答疑 →</button>`;
  }
  if(reviewBox){
    const version=teacherTaskState.version,versions=teacherTaskState.versions||[];
    reviewBox.innerHTML=version?`<div class="report-facts"><span><b>v${version.versionNo}</b> 当前版本</span><span><b>${version.assignedCount??0}</b> 已分配</span><span><b>${version.completedCount??0}</b> 已完成</span><span><b>${valueText(version.completionRate,'%')}</b> 完成率</span></div><p><b>当前状态：</b>${esc({draft:'草案',published:'进行中',superseded:'已替代',revoked:'已撤回'}[version.status]||version.status)} · 历史 ${versions.length} 个版本</p><button class="link-btn" onclick="openView('tasks')">查看任务与反馈 →</button>`:'<p class="data-empty">当前班级尚无任务复盘数据。</p>';
  }
}
...
addEventListener('zhixue:teacher-qa-state',event=>{teacherQaState=event.detail||teacherQaState;renderTeacherReports()});
...
function workspaceHash(view){const params=new URLSearchParams();if(state.activeCourse)params.set('course',state.activeCourse);if(state.activeClass)params.set('class',state.activeClass);if(view==='studentDetail'&&selectedStudentNo)params.set('student',selectedStudentNo);return`#${PAGE}/${view}${params.size?`?${params}`:''}`}
function currentView(){return document.querySelector('.view.on')?.id||roleViews[PAGE]?.[0]}
window.openView=function(id,options={}){if(!roleViews[PAGE]?.includes(id))id=roleViews[PAGE][0];document.querySelectorAll('.view').forEach(x=>x.classList.toggle('on',x.id===id));document.querySelectorAll('.menu').forEach(x=>{const active=x.dataset.view===id||(id==='studentDetail'&&x.dataset.view==='students');x.classList.toggle('on',active);if(active)x.scrollIntoView?.({block:'nearest',inline:'nearest'})});const copy=viewCopy[id];if(copy){const title=document.querySelector('.page-title'),desc=document.querySelector('.page-desc'),crumb=document.querySelector('#crumbName');if(title)title.textContent=copy[0];if(desc)desc.textContent=copy[1];if(crumb)crumb.textContent=copy[0]}if(!options.fromHistory){const method=options.replace?'replaceState':'pushState';history[method]({view:id,course:state.activeCourse,classId:state.activeClass,student:selectedStudentNo},'',workspaceHash(id))}if(!options.keepScroll)scrollTo({top:0,behavior:options.fromHistory?'auto':'smooth'});renderAll();if(id==='assistant'||id==='history')window.ZhixueQaClient?.refreshStudent();if(id==='inbox'||id==='reports')window.ZhixueQaClient?.refreshTeacher();if(id==='tasks')window.ZhixueTaskClient?.setTeacherContext(currentContextKey(),m2State.analysisRunId);if(id==='studentTasks'||id==='history')window.ZhixueTaskClient?.setStudentCourse(course()?.offeringId);if(id==='students')void loadTeacherStudents();if(id==='reports')renderTeacherReports()}
async function restoreWorkspaceRoute(replace=false){if(!roleViews[PAGE])return;const route=readWorkspaceRoute(),beforeCourse=state.activeCourse,beforeClass=state.activeClass,requestedCourse=route.params?.get('course'),requestedClass=route.params?.get('class'),requestedStudent=route.params?.get('student');if(requestedCourse&&state.courses.some(x=>x.id===requestedCourse))state.activeCourse=requestedCourse;const selected=course();if(PAGE==='teacher'&&requestedClass&&selected?.classes.some(x=>x.id===requestedClass))state.activeClass=requestedClass;else if(!selected?.classes.some(x=>x.id===state.activeClass))state.activeClass=selected?.classes[0]?.id||'';const target=route.view||roleViews[PAGE][0];let resolvedTarget=target;if(beforeCourse!==state.activeCourse||beforeClass!==state.activeClass){saveState();if(PAGE==='teacher')await loadTeacherDashboard();else applyStudentCourse(state.activeCourse)}if(PAGE==='teacher'&&target==='studentDetail'&&requestedStudent){const loaded=await observeStudent(requestedStudent,{navigate:false});resolvedTarget=loaded?'studentDetail':'students'}openView(resolvedTarget,{fromHistory:true,replace,keepScroll:true});if(replace)history.replaceState({view:resolvedTarget,course:state.activeCourse,classId:state.activeClass,student:selectedStudentNo},'',workspaceHash(resolvedTarget))}
...
function renderTeacherStudents(){const box=document.querySelector('#teacherStudentList');if(!box)return;const q=(document.querySelector('#studentSearch')?.value||'').trim().toLowerCase(),risk=document.querySelector('#studentRiskFilter')?.value||'all';const rows=teacherStudents.filter(x=>(!q||`${x.studentNo} ${x.displayName} ${(x.weakPoints||[]).join(' ')}`.toLowerCase().includes(q))&&(risk==='all'||risk==='none'&&!x.risk||x.risk?.level===risk));setText('#studentListSummary',`当前 ${rows.length} / ${teacherStudents.length} 名学生 · 此处仅显示概览数据`);box.innerHTML=rows.length?rows.map(x=>`<button class="teacher-student-row ${selectedStudentNo===x.studentNo?'on':''}" data-observe-student="${esc(x.studentNo)}"><span><b>${esc(x.displayName)}</b><small>${esc(x.studentNo)} · 最近活跃 ${esc(x.lastActiveAt?new Date(x.lastActiveAt).toLocaleDateString('zh-CN'):'暂无')}</small></span><em>成绩 ${x.score??'--'}</em><em>掌握 ${x.mastery??'--'}%</em><em>任务 ${x.taskCompletionRate??'--'}%</em><i class="risk-${esc(x.risk?.level||'none')}">${esc(x.risk?.level||'正常')}</i><strong>查看详情 →</strong></button>`).join(''):'<p class="data-empty">当前筛选条件下没有学生。</p>'}
async function observeStudent(studentNo,{navigate=true}={}){const box=document.querySelector('#studentObservation');selectedStudentNo=studentNo;selectedObservation=null;personalPlanDraft=null;if(navigate)openView('studentDetail');if(box)box.innerHTML='<p class="data-empty">正在验证教学关系并读取学生详细学习数据与智能分析…</p>';try{const payload=await fetchDatabase(`/api/teacher/students/${encodeURIComponent(studentNo)}/observation?context=${encodeURIComponent(currentContextKey())}`);selectedObservation=payload.data;personalPlanDraft=payload.data.plan||null;renderTeacherStudents();renderObservation();renderPersonalPlan();return true}catch(error){selectedStudentNo='';selectedObservation=null;if(box)box.innerHTML=`<div class="empty"><b>无法查看该学生</b>${esc(error.message)}；没有读取或保存越权数据。请返回概览重试。</div>`;return false}}
function renderObservation(){const box=document.querySelector('#studentObservation'),o=selectedObservation;if(!box||!o)return;const r=o.shared.required,opt=o.shared.optional,p=o.profile.profile,knowledge=r.knowledge||[],answers=opt.answerText||[],qa=opt.qaContent||[],tags=opt.goalsPreferences?.profileTags||[];setText('#observationTitle',`${o.student.displayName} · 学习详情与智能分析`);box.innerHTML=`<div class="observation-banner"><b>任课教师详情视角</b><span>${esc(o.course.courseName)} · ${esc(o.student.className)} · ${esc(o.student.studentNo)} · 学习数据按教学关系自动开放</span></div><div class="observation-metrics"><div><span>课程成绩</span><b>${r.score??'--'}</b></div><div><span>综合掌握</span><b>${r.mastery??'--'}%</b></div><div><span>出勤率</span><b>${r.attendanceRate??'--'}%</b></div><div><span>任务完成</span><b>${r.tasks.completionRate??'--'}%</b></div></div><div class="student-detail-grid"><section><h4>智能诊断</h4><p class="analysis-callout"><b>系统结论：</b>${esc(p.probable_cause||'当前证据不足，建议继续观察。')}<br>错题 ${r.wrongBook.total} 条，已复习 ${r.wrongBook.reviewed} 条；逾期 ${r.tasks.overdue} 项。</p><h4>知识点详细掌握</h4><div class="detail-mastery">${knowledge.length?masteryHTML(knowledge.map((x,index)=>({...x,color:['#1684ff','#23c983','#ffb52b'][index%3]}))):'<p class="data-empty">暂无知识点证据</p>'}</div><h4>画像标签</h4><div class="observation-points">${tags.length?tags.map(tag=>`<span>${esc(tag)}</span>`).join(''):'<span>暂无额外标签</span>'}</div></section><section><h4>近期错题与作答</h4><div class="detail-records">${answers.length?answers.map(x=>`<article><b>${esc(x.prompt)}</b><p>作答：${esc(x.answer_text||'未记录')} · ${esc(x.error_type||'待分析')}</p></article>`).join(''):'<p class="data-empty">暂无近期错题明细</p>'}</div><h4>课程答疑记录</h4><div class="detail-records">${qa.length?qa.map(x=>`<article><b>${esc(x.question)}</b><p>${esc(x.answer||'等待回答')} · ${esc(x.status||'')}</p></article>`).join(''):'<p class="data-empty">暂无课程答疑记录</p>'}</div><p class="auto-access-note">行为、作答、答疑和学习目标仅按当前教学关系向任课教师开放，其他学生无法访问。</p></section></div>`}
function renderPersonalPlan(){const panel=document.querySelector('#personalPlanEditor');if(!panel)return;if(!personalPlanDraft){panel.hidden=true;return}panel.hidden=false;document.querySelector('#personalPlanGoal').value=personalPlanDraft.goal;document.querySelector('#personalPlanRoute').value=personalPlanDraft.route;document.querySelector('#personalPlanDue').value=personalPlanDraft.dueAt?.slice(0,16)||'';setText('#personalPlanStatus',`v${personalPlanDraft.versionNo} · 自动生效`);document.querySelector('#personalPlanTasks').innerHTML=personalPlanDraft.tasks.map((task,index)=>`<article><b>${index+1}. ${esc(task.title)}</b><span>${esc(task.difficulty)} · ${task.duration_minutes} 分钟 · ${esc(task.knowledge_point)}</span><p>${esc(task.reason)}</p></article>`).join('')}
async function savePersonalPlan(){if(!personalPlanDraft)return;try{const dueValue=document.querySelector('#personalPlanDue').value,body={context:currentContextKey(),goal:document.querySelector('#personalPlanGoal').value,route:document.querySelector('#personalPlanRoute').value,dueAt:new Date(dueValue).toISOString(),tasks:personalPlanDraft.tasks};const response=await window.ZhixueApi.apiFetch(`/api/plans/${personalPlanDraft.planId}`,{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify(body)}),payload=await response.json();if(!response.ok||!payload.success)throw new Error(payload.message||'保存失败');personalPlanDraft=payload.data;renderPersonalPlan();toast('个性化方案调整已保存并立即生效')}catch(error){toast(`${error.message}；原方案未被覆盖`)}}
document.querySelector('#studentSearch')?.addEventListener('input',renderTeacherStudents);document.querySelector('#studentRiskFilter')?.addEventListener('change',renderTeacherStudents);document.querySelector('#refreshStudents')?.addEventListener('click',loadTeacherStudents);document.addEventListener('click',event=>{const target=event.target.closest('[data-observe-student]');if(target)void observeStudent(target.dataset.observeStudent)});document.querySelector('#backToStudentOverview')?.addEventListener('click',()=>openView('students'));document.querySelector('#savePersonalPlan')?.addEventListener('click',savePersonalPlan);
async function loadActivePersonalPlan(){const box=document.querySelector('#activePersonalPlan');if(!box||PAGE!=='student'||!course()?.offeringId)return;try{const plans=(await fetchDatabase(`/api/student/plans/active?offeringId=${course().offeringId}`)).data||[],plan=plans[0];box.innerHTML=plan?`<div class="card-head"><div><h3>今日个人学习计划</h3><small>系统按当前学习数据自动生成 · v${plan.versionNo}</small></div><span>${new Date(plan.dueAt)<new Date()?'已逾期':'截止 '+new Date(plan.dueAt).toLocaleDateString('zh-CN')}</span></div><h4>${esc(plan.goal)}</h4><p><b>学习路线：</b>${esc(plan.route)}</p><div class="personal-plan-tasks">${plan.tasks.map((task,index)=>`<article><b>${index+1}. ${esc(task.title)}</b><span>${esc(task.difficulty)} · ${task.duration_minutes} 分钟</span><p>${esc(task.reason)}</p></article>`).join('')}</div><details><summary>为什么推荐</summary><pre>${esc(JSON.stringify(plan.whyRecommended,null,2))}</pre></details><p class="auto-access-note">方案已自动生效；任课教师可以查看并根据教学判断调整。</p>`:'<p class="data-empty">当前数据暂不足以生成个性化方案，请稍后重试。</p>'}catch(error){box.innerHTML=`<div class="empty"><b>个人计划加载失败</b>${esc(error.message)}；请点击课程或刷新页面重试。</div>`}}
...
function renderAll(){fillMastery();renderCourseSelectors();renderTeacherCourses();renderStudentCourses();renderInbox();renderStudentTasks();renderTimeline();renderTeacherLiveState();renderStudentLiveState();renderTeacherReports()}
