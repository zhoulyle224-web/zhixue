...
function renderTeacherLiveState(){
  ...
  if(!batch)items.push({view:'import',title:'导入当前班级数据',detail:'尚无当前批次；导入并质检后才能生成研判。'});
  else if(batch.status==='validated')items.push({view:'import',title:'确认当前批次质检结果',detail:`有效 ${quality.validRows} 行，排除 ${quality.invalidRows} 行，警告 ${quality.warningRows} 行。`});
...
const viewCopy={
  teacherHome:['工作台首页','从班级数据出发，完成研判、教学调整与反馈复盘。'],classes:['课程与班级','查看服务端授权的课程与实际教学班级。'],import:['数据导入与质检','导入匿名学习数据，并在分析前处理数据质量问题。'],analysis:['学情智能研判','查看班级、学生与知识点掌握情况及其依据。'],tasks:['分层任务发布','确认智能体生成的差异化任务并发布到学生端。'],inbox:['学生问题与反馈','处理高频问题，并将结果重新纳入教学研判。'],reports:['报告与复盘','导出学情、答疑记录与教学改进建议。'],
  studentHome:['学习首页','查看当前课程、个人薄弱点和待完成任务。'],courses:['我的课程','查看服务端授权课程并切换独立的学习空间。'],portrait:['个人学情画像','了解掌握情况、证据来源与下一步建议。'],assistant:['课程智能答疑','基于课程资料提问，答案附带引用依据。'],studentTasks:['个性化任务','完成教师确认发布的专属学习任务。'],history:['学习记录与反馈','回看学习过程和掌握度变化。']
};
const roleViews={teacher:['teacherHome','classes','import','analysis','tasks','inbox','reports'],student:['studentHome','courses','portrait','assistant','studentTasks','history']};
