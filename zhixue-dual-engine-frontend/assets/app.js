...
function setDatabaseStatus(mode,meta={}){const el=document.querySelector('#databaseStatus');if(!el)return;el.classList.remove('loading','fallback');if(mode==='local'){el.textContent=`● 本地 SQLite · ${Number(meta.recordCount||0).toLocaleString()} 条`;el.title=`本地只读数据库，共 ${meta.tableCount||0} 类业务表，断网可用`}else if(mode==='database'){el.textContent=`● 云端数据库 · ${Number(meta.recordCount||0).toLocaleString()} 条`;el.title=`D1 数据源，${meta.tableCount||0} 类业务表`}else if(mode==='fallback'){el.classList.add('fallback');el.textContent='● 本地数据快照';el.title='当前为纯静态模式，已使用同源脱敏快照，答疑由本地 Skill 兜底'}else{el.classList.add('loading');el.innerHTML='<i></i>数据库连接中'}}
...
async function loadTeacherDashboard(){if(PAGE!=='teacher')return;const key=currentContextKey(),token=++m2Token;++m2RequestToken;m2State={context:key,batch:null,analysis:null,issues:[],preview:[],qualityOverride:null,error:'',busy:false,offline:false};resetM2Visuals();renderM2State();if(!key){setText('#qualitySummary','请先选择数据库中的课程和班级');return}let data;try{data=(await fetchDatabase(`/api/dashboard?audience=teacher&context=${encodeURIComponent(key)}`)).data}catch{data=databasePayload?.teacher?.[key]}if(token!==m2Token)return;if(data){knowledge=data.knowledge||knowledge;renderAll();setText('#studentCount',data.studentCount);setText('#masteryAvg',`${data.averageMastery}%`);setText('#weakCount',data.weakCount);setText('#pendingCount',data.pendingCount);setText('#classAverage',data.averageScore);setText('#classPassRate',`${data.passRate}%`);setText('#classAttendance',`${data.attendanceRate}%`);setText('#classRiskCount',data.riskCount);setText('#tierTotal',data.studentCount);setText('#tierA',data.tiers?.['拓展组']||0);setText('#tierB',data.tiers?.['提升组']||0);setText('#tierC',data.tiers?.['巩固组']||0);const topics=document.querySelector('#databaseHotTopics');if(topics)topics.innerHTML=(data.hotTopics||[]).map(x=>`<span>${esc(x.name)} <b>${x.count}</b></span>`).join('')||'<span>暂无答疑记录</span>'}await restoreM2(key,token)}
...
function localAssistantAnswer(q){const result=window.ZhixueSkillTutor?.answer({student_question:q,course_name:course()?.name||'本课程'});if(result){return {text:result.answer_content,source:(result._refs||[]).join('；'),status:result.answer_status,guides:result.guide_questions||[]}}return {text:'当前课程资料中没有足够内容直接支持这个问题。已把问题记录到当前课程，等待教师确认。',source:'课程资料检索',status:'待人工处理',guides:[]}}
async function assistantAnswer(q){try{const response=await fetch('/api/qa',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({question:q,courseName:course()?.name||'本课程',role:'student',context:currentContextKey()})});const payload=await response.json();if(!response.ok||!payload.success)throw new Error(payload.message||'skill unavailable');const data=payload.data;return {text:data.answer_content,source:(data._refs||[]).join('；'),status:data.answer_status,guides:data.guide_questions||[]}}catch{return localAssistantAnswer(q)}}
function chatText(value){return esc(value).replace(/^## (.*)$/gm,'<b>$1</b>').replace(/\*\*(.*?)\*\*/g,'<b>$1</b>').replace(/\n/g,'<br>')}
function addChat(role,content,source='',guides=[]){const box=document.querySelector('#chatMessages');if(!box)return;const div=document.createElement('div');div.className=`chat ${role}`;div.innerHTML=`<span>${role==='ai'?'AI':'我'}</span><div><b>${role==='ai'?'课程学习助手':'我的问题'}</b><p>${chatText(content)}</p>${source?`<button class="citation">依据：${esc(source)}</button>`:''}${guides.length?`<div class="guide-questions">${guides.map(x=>`<span>${esc(x)}</span>`).join('')}</div>`:''}</div>`;box.appendChild(div);box.scrollTop=box.scrollHeight}
async function sendQuestion(){const input=document.querySelector('#askInput'),q=input?.value.trim();if(!q){toast('请先输入课程问题');return}addChat('user',q);input.value='';const pending={id:uid(),courseId:state.activeCourse,text:q,status:'pending',answer:'',created:new Date().toISOString(),source:'课程智能答疑'};state.questions.unshift(pending);saveState();const a=await assistantAnswer(q);pending.answer=a.text;pending.source=a.source;pending.status=a.status==='已解答'?'done':'pending';saveState();addChat('ai',a.text,a.source,a.guides);toast(a.status==='已解答'?'回答已生成，并附带课程资料依据':'资料不足，已转教师确认')}
