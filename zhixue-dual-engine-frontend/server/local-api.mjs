...
async function handleApi(request, url, runtimeStore) {
  ...
  if (request.method === "POST" && url.pathname === "/api/qa") {
    ...
    const courseName = cleanText(body.courseName || "本课程", 80);
    if (!question) {
      return json(
        {
          success: false,
          code: "EMPTY_QUESTION",
          message: "请输入需要解答的课程问题。",
        },
        400,
      );
    }
    if (PROMPT_INJECTION.some((pattern) => pattern.test(question))) {
      await recordAudit({
        actorRole: cleanText(body.role || "student", 20),
      ...
      return json(
        {
          ...
          message: "问题包含越权指令，已拦截。请改为询问课程知识点。",
        },
        400,
      );
    }
    const safeQuestion = redactText(question);
    const answer = skills.tutor.answer({
      student_question: safeQuestion,
      course_name: courseName,
      question_context: cleanText(body.context, 120),
      student_level: cleanText(body.studentLevel || "普通", 20),
    });
    await recordAudit({
      actorRole: cleanText(body.role || "student", 20),
      action: "qa_answer",
      objectType: "skill",
      objectRef: "course-ai-tutor",
      result: answer.answer_status,
      redacted: safeQuestion !== question,
    });
    return json({
      success: true,
      source: "local_skill",
      data: {
        ...
        pii_redacted: safeQuestion !== question,
      },
    });
  }
...
export function createZhixueServer({ runtimeDbPath } = {}) {
  const runtimeStore = createRuntimeStore(runtimeDbPath);
  const server = createServer(async (request, response) => {
    try {
      const url = new URL(request.url || "/", `http://${request.headers.host || "localhost"}`);
      const result = url.pathname.startsWith("/api/")
        ? await handleApi(request, url, runtimeStore)
  ...
  server.on("close", () => runtimeStore.close());
  return server;
}
